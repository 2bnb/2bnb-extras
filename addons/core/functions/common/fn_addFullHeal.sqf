...
private _action = [];
{
	...
	if ({isMultiplayer} && {{_object in _x} count bnb_e_EHIDs} == 0) then {
		_id = ["addFullHeal", [_object]] call CBA_fnc_globalEventJIP;
		[_id, _object] call CBA_fnc_removeGlobalEventJIP;

		bnb_e_EHIDs pushBack [_id, _object];
		publicVariable bnb_e_EHIDs;
		[format["Event ""addFullHeal"":%1 raised for %2", _id, _object], "core\functions\common\fn_addFulleHeal.sqf"] call bnb_e_core_fnc_log;
	};

	if ({(_x select 0) select 0 isEqualTo "ace_arsenal_interaction"} count (_object getVariable ["ace_interact_menu_actions", []]) == 0) then {
		_action = [
			"FullHeal",
			"Full Heal",
		...
		// [_object, 0, ["ACE_MainActions"], _action] remoteExecCall ["ace_interact_menu_fnc_addActionToObject"];
